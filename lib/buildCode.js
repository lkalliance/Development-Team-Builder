...
function buildCard(employee) {
    ...
    const card = `<div class="card"><div class="card-body"><h5 class="card-title">${employee.name}<span class="icon ${translator[role].abbrev}">${translator[role].abbrev.toUpperCase()}</span></h5></div><ul class="list-group list-group-flush"><li class="list-group-item id"><span class="label">id: </span>${employee.id}</li><li class="list-group-item mail"><span class="label">email: </span><a href="mailto:${employee.email}">${employee.email}</a></li><li class="list-group-item ${translator[role].style}"><span class="label">${translator[role].label}</span>${extraLink[0]}${employee[translator[role].extra]}${extraLink[1]}</li></ul></div>`;
