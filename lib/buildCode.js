...
const translator = {
    "Manager": { abbrev: "MGR", extra: "officeNumber", style: "phone" },
...
function buildCard(employee) {
    ...
    const extraLink = ( role == "Engineer" ) ? [`<a href="https://www.github.com/${employee.gitHub}">`, `</a>`] : [``,``];

    const card = `<div class="card"><div class="card-body"><h5 class="card-title">${employee.name}<span class="icon">${translator[role].abbrev}</span></h5></div><ul class="list-group list-group-flush"><li class="list-group-item id">${employee.id}</li><li class="list-group-item mail"><a href="mailto:${employee.email}">${employee.email}</a></li><li class="list-group-item ${translator[role].style}">${extraLink[0]}${employee[translator[role].extra]}${extraLink[1]}</li></ul></div>`;
