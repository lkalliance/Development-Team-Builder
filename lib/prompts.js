...
function createEngineer(roster) {
    ...
    console.log(`\n\x1b[33mAdd an \x1b[33;1mengineer\x1b[33m to your team:\n`);
    // first add a validation for unique ID, using current roster
    const idVal = new Promise( (resolve, reject) => {
        addIdValidation(roster);
        resolve(true);
...
function createIntern(roster) {
    ...
    console.log(`\n\x1b[33mAdd an \x1b[33;1mintern\x1b[33m to your team:\n`)
    // first add a validation for unique ID, using current roster
    const idVal = new Promise( (resolve, reject) => {
        addIdValidation(roster);
        resolve(true);
...
function another(roster) {
    ...
    inquirer.prompt(
        {
            name: "continue",
            message: "\x1b[33mAdd another employee, or indicate you're done",
            type: "list",
        ...
        // if yes, send to the appropriate constructor
            ...
            console.log(`\x1b[33;1m\nYou have finished with ${roster.length} employees entered.\n`);
...
function addIdValidation(roster) {
    ...
    promptArr[1].validate = (input) => {
        ...
        for(employee of roster) {
            if (input == employee.id) return `\x1b[33mThis id is already assigned to a different employee`;
        }
        if( input.trim().length > 0 && unique ) return true;
        else return "\x1b[33mThe id must have at least one non-space character";
    }
}
